...
xgettext --output=po/popcorn.pot --files-from=po/POTFILES.in --from-code=UTF-8 --add-comments --keyword=_ --keyword=C_:1c,2 --keyword=ngettext:1,2

echo -e "\n---🍿 Popcorn Translation Tool 🍿---\n"
...
if [[ -f "$po_file" ]]; then
    echo -e "\nUpdating $lang..."
    msgmerge --no-fuzzy-matching --update --backup=none "$po_file" po/popcorn.pot
    ...
    if [[ "$answer" =~ ^[Yy]$ ]]; then
    	echo ""
        msginit --locale="$lang" --input=po/popcorn.pot --output-file="$po_file"
