...
class EpisodeButton(Gtk.Box):
    ...
    def format_subtitle(self, obj, season, episode, episode_name) -> str:
        return f"{_('S{} E{}').format(season, episode)} - {GLib.markup_escape_text(episode_name)}"
    @Gtk.Template.Callback()
    ...
    def build_context_menu(self) -> ContextMenu:
        ...
        menu.add_row(ContextMenuRow(
            title=_("Resume Episode") if model.get_property('Progress') > 0 else _("Play Episode"),
            icon_name="media-playback-start-symbolic",
        ))
        menu.box.get_last_child().connect('activated', lambda *_: self.activate_action(
        ...
        series_row = ContextMenuRow(
            title=_("Go to Series"),
            icon_name="go-next-symbolic",
        )
        series_row.connect('activated', lambda *_: self.activate_action(
            'app.show_series', GLib.Variant('s', model.get_property('SeriesId'))
        ))
        menu.add_row(series_row)
